...
#else
static void multi_timeout(struct timer_list *tl);
#endif
	...
	switch (cmd) {
		...
		case SETFCR:
			if (info->port.type == PORT_16C105X || info->port.type == PORT_16C105XA){
				...
				serial_out((struct mp_port *)state->port,2,(unsigned long)arg);
			}
...
static void autoconfig(struct mp_port *mtpt, unsigned int probeflags)
{
	...
	u_type = (b_ret & 0xf0) >> 4;
    DEBUG_AUTOCONF("before: type=%d\n", mtpt->port.type);
    DEBUG_AUTOCONF("utype=%x\n", b_ret);
...
static void multi_timeout(struct timer_list *tl)
{
	struct mp_port *mtpt = (struct mp_port *)((void*)tl - offsetof(struct mp_port, timer));
#endif
